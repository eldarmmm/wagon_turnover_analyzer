        ...
        try:
            ...
            df = pd.read_sql(f"""
                ...
                WHERE c.DateOper >= '{self.date_from}'
                  AND c.DateOper <= DATEADD(day, 90, '{self.date_to}')
            ...
            # Сортируем перед .first() — порядок строк из SQL не гарантирован
            first_isfull = (df.sort_values('Дата_операции')
                              .groupby('Номер_вагона')['IsFull']
                              .first())
            loaded_at_start = first_isfull[first_isfull == 1].index.tolist()
            skip_first_trip = set()
            lb_df_dict      = {}  # {номер_вагона: DataFrame} — lookback строки для group
            ...
            if loaded_at_start:
                ...
                try:
                    ...
                    for car, grp in lb_df.groupby('Номер_вагона'):
                        ...
                        # Сохраняем для добавления в group перед find_turnovers
                        lb_df_dict[car] = grp
                        # Определяем skip_first: отправление с погрузки ДО date_from?
                        load_idx = None
                    ...
                    self.emit_log(f"Рейсов вне периода (skip_first): {len(skip_first_trip):,}")
                    self.emit_log(f"Вагонов с lookback-данными: {len(lb_df_dict):,}")
                except Exception as e:
            ...
            for idx, (wagon_num, group) in enumerate(groups):
                # Добавляем lookback-данные (строки до date_from) в начало group.
                # Это позволяет find_turnovers восстановить рейс начатый до периода.
                wkey = str(wagon_num).strip()
                if wkey in lb_df_dict:
                    group = pd.concat([lb_df_dict[wkey], group], ignore_index=True)
                    group = group.sort_values('Дата_операции').reset_index(drop=True)
